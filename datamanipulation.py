...
def main(argv):
        ...
        for i in range(0, len(ayAverage)):
                ...
                calcRawAngleValues.append(-(angle+1.5))

        calcAvgAngleValues = []
        for i in range(0, len(ayAverage)):
                angle = calcAngle(axAverage[i], ayAverage[i], azAverage[i])
                calcAvgAngleValues.append(-(angle+1.5))

        # calcAvgAngleValues = generateAveragesV2(0, calcAvgAngleValues, 15, )

        print("ANGLE AVERAGE")
        angleAverage = generateAverages(-2, calcAvgAngleValues, 10, 5, 4, 10, 4, -2)
        ...
        plt.plot(np.array(xNumber), np.array(calcAvgAngleValues),                             
                'orange',                       # colour
                linestyle='--',                    # line style
...
# ANGLE VALUES
        ...
        i = 0
        # Grab the values
        with open("finalDownhill1.txt") as f:
                for line in f:
                        if (len(line) > 1):
                                splitLine = line.split(",")
                                if (splitLine[0] == "ANGLE"):
        ...
        for i in range(0, len(ayAverage)):
                angle = calcAngle(axValues[i], ayValues[i], azValues[i])
                calcRawAngleValues.append(-(angle))

        calcAvgAngleValues = []
        for i in range(0, len(ayAverage)):
                angle = calcAngle(axAverage[i], ayAverage[i], azAverage[i])
                calcAvgAngleValues.append(-(angle))
...
# ANGLE VALUES
        ...
        i = 0
        # Grab the values
        with open("finalUphill1.txt") as f:
                for line in f:
                        if (len(line) > 1):
                                splitLine = line.split(",")
                                if (splitLine[0] == "ANGLE"):
